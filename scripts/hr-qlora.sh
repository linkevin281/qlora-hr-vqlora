...
usage() { echo "Usage: $0 -r <lora rank> -c <codebook size> -l <learning rate (og: 0.0002)> [-n <no. codebook layers>] [-e <extra name>] [-g <gpu>]" 1>&2; exit 1; }

while getopts ":c:r:l:n:e:g:" o; do
    case "${o}" in
        c)
...
CUDA_VISIBLE_DEVICES="${gpu}" python qlora.py \
